...
void TimerEnvInit (TTimer *pThis)
{
	//pThis = malloc(sizeof(TTimer));
	assert (pThis != 0);
...
boolean TimerInitialize (TTimer *pThis)
{
	...
	//TODO
	//TimerEnvInit(pThis);
	ConnectTimerInterrupt (ARM_IRQ_TIMER1, TimerInterruptHandler, pThis);

	DataMemBarrier ();

	write32 (ARM_SYSTIMER_CLO, -(30 * CLOCKHZ));	// timer wraps soon, to check for problems

	write32 (ARM_SYSTIMER_C1, read32 (ARM_SYSTIMER_CLO) + (CLOCKHZ / HZ)*5);
...
void TimerInterruptHandler (void *pParam)
{
	...
	assert (read32 (ARM_SYSTIMER_CS) & (1 << 1));

	u32 nCompare = read32 (ARM_SYSTIMER_C1) + (CLOCKHZ / HZ)*5;
	write32 (ARM_SYSTIMER_C1, nCompare);
	if (nCompare < read32 (ARM_SYSTIMER_CLO))			// time may drift
	{
		nCompare = read32 (ARM_SYSTIMER_CLO) + (CLOCKHZ / HZ)*5;
		write32 (ARM_SYSTIMER_C1, nCompare);
	}
